<script lang="ts">
	...
	// 검색 우선순위 계산 함수
	function getSearchPriority(album: (typeof albums)[number], query: string): number {
		if (!query) return 0;
	...
	function handleAssignMember(albumId: string) {
		// 실제 멤버 배정 로직 구현 예정
		moreMenuOpenId = null;
	}

	function handleCopyShareLink(albumId: string) {
		// 실제 공유 링크 복사 로직 구현 예정
		moreMenuOpenId = null;
	}
