<script lang="ts">
	...
	// URL 파라미터에서 ID 가져오기
	const revenueId = $derived($page.params.id);

	// 수익 데이터
	let revenue = $state<any>(null);
	...
	$effect(() => {
		(async () => {
			if (!revenueId) {
				toast.add('수익 ID가 없습니다.', 'error', 3000);
				goto('/revenue');
				return;
			}
			...
			try {
				...
				if (data.ok && data.data) {
					...
					if (found) {
						...
						toast.add('수익 데이터를 찾을 수 없습니다.', 'error', 3000);
						goto('/revenue');
					}
				} else {
					toast.add('수익 데이터를 불러오는데 실패했습니다.', 'error', 3000);
					goto('/revenue');
				}
			} catch (error) {
				console.error('수익 로드 오류:', error);
				toast.add('수익 데이터를 불러오는 중 오류가 발생했습니다.', 'error', 5000);
				goto('/revenue');
			} finally {
				loading = false;
			}
	...
	async function handleSubmit() {
		...
		try {
			...
			// 	const errorMessage = result.error?.message || '수익 수정에 실패했습니다.';
			// 	throw new Error(errorMessage);
			// }

			// 임시: 성공 메시지만 표시
			toast.add('수익이 성공적으로 수정되었습니다. (API 구현 대기 중)', 'success', 3000);
...
<PageContent>
	...
	{:else if !revenue}
		<div class="flex items-center justify-center py-12">
			<p class="text-text-muted">수익 데이터를 찾을 수 없습니다.</p>
		</div>
	{:else}
		<!-- 수정 폼 -->
