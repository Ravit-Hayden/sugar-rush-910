			...
			<div class="flex-1 flex flex-col lg:flex-row lg:h-48">
					...
					<div class="flex justify-center gap-1 px-2 items-end">
						{#each weeklyData as item}
							<div class="flex flex-col items-center w-12">
								<div class="{item.color} rounded-t-sm" style="height: {item.height}px; width: 12px;"></div>
								<span class="mt-1 text-xs text-text-muted">{item.day}</span>
							</div>
						{/each}
					</div>
				</div>
				...
				<div class="hidden lg:flex lg:flex-1 lg:flex-col lg:justify-center lg:pl-10">
					<div class="space-y-3">
						...
						<div class="text-xs text-ok-fg -mt-6">일평균</div>
