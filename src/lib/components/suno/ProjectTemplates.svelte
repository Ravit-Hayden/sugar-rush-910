<script lang="ts">
	import { 
		...
		Zap as ZapIcon, Activity, Cable, Siren, Euro, CloudFog,
		// 새 장르 아이콘
	...
	const filteredTemplates = $derived.by(() => {
		...
		if (selectedCategory === 'favorite') {
			result = templates.filter(t => t.isFavorite);
		} else {
			result = templates.filter(t => t.category === selectedCategory);
		}
		// 이름순 정렬 (A-Z)
...
<div class="bg-surface-1 rounded-lg border border-border-subtle overflow-hidden h-[70vh] flex flex-col">
	...
	<div class="px-6 py-4 border-t border-border-subtle flex-shrink-0">
		<button
			type="button"
			onclick={() => handleSelect({ id: 'blank', name: '빈 프로젝트', description: '', icon: Droplet, color: '', defaultStyles: '', defaultExclude: '', defaultVocalGender: 'Female', tags: [], category: 'special' })}
