...
CREATE TABLE IF NOT EXISTS suno_subscriptions (id TEXT PRIMARY KEY, plan_type TEXT NOT NULL, billing_date INTEGER NOT NULL, monthly_credits INTEGER NOT NULL, remaining_credits INTEGER NOT NULL, updated_by TEXT NOT NULL DEFAULT 'El', last_updated TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS suno_subscription_logs (id TEXT PRIMARY KEY, updated_by TEXT NOT NULL, updated_at TEXT NOT NULL, before_json TEXT, after_json TEXT NOT NULL);
CREATE INDEX IF NOT EXISTS idx_suno_subscription_logs_updated_at ON suno_subscription_logs(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_suno_subscription_logs_updated_by_updated_at ON suno_subscription_logs(updated_by, updated_at DESC);
CREATE TABLE IF NOT EXISTS artists (id TEXT PRIMARY KEY, name TEXT NOT NULL UNIQUE, description TEXT, photo_url TEXT, website TEXT, email TEXT, instagram TEXT, twitter TEXT, youtube TEXT, genre TEXT, debut_date TEXT, agency TEXT, country TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);-- 샘플 아티스트 데이터
