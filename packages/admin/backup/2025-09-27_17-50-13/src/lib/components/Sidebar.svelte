<script lang="ts">
	...
	<aside class="top-24 fixed left-0 h-[calc(100vh-96px)] bg-surface-2 border-r border-border-subtle z-40 transition-all duration-500 ease-in-out {sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 {sidebarCollapsed ? 'w-16' : 'w-[240px]'}" style="transition: width 500ms cubic-bezier(0.4, 0, 0.2, 1), transform 500ms cubic-bezier(0.4, 0, 0.2, 1);">
			...
			{#each menuItems as item (item.label)}
				...
				<li class="overflow-hidden">
					<a
						...
						class="flex items-center h-12 w-full text-text-base hover:bg-hover-cyan hover:text-black transition-colors duration-200 {isActive(item.href) ? 'bg-brand-pink text-white' : ''} px-6"
						style="outline: none;"
						title={sidebarCollapsed ? item.label : ''}
					>
						<div class="flex items-center gap-3 w-full">
							<IconComponent size={16} class="flex-shrink-0" />
							{#if !sidebarCollapsed}
								<span class="text-sm whitespace-nowrap transition-opacity duration-300">{item.label}</span>
							{/if}
						</div>
					</a>
				</li>
