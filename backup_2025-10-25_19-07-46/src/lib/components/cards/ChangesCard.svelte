...
<div class="h-[396px] flex flex-col justify-between p-5 rounded-lg bg-surface-2 border border-border-subtle overflow-hidden pt-[24px]">
	<div>
		...
		<div class="flex items-center justify-between mb-4">
			...
			<button class="inline-flex items-center justify-center w-5 h-5 rounded-full hover:bg-surface-1 transition-colors ml-2" aria-label="정보" title="편집·배포·정책 변경 요약" type="button">
				...
				{#each displayChanges.slice(0, 4) as change, i (change.id)}
					...
					>
						...
						<span class="flex-shrink-0 flex items-center gap-x-2 ml-2">
							<span class="text-xs text-text-muted whitespace-nowrap">{change.time}</span>
						</span>
					</a>
				{/each}
